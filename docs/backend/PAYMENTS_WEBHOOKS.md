...
- Queues a customer email job for successful payments.
- Queues a customer email job for failed payments with the failure reason.
...
- `SendPaymentSucceededEmail` is queued after a PayFast `COMPLETE` webhook and sends `PaymentSucceededMail`.
- `SendPaymentFailedEmail` is queued after a PayFast `FAILED` or `CANCELLED` webhook and sends `PaymentFailedMail`.
- Duplicate webhook events do not queue duplicate emails.
